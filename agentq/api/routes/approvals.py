...
async def decide_approval(approval_id: str, body: ApprovalDecision, session: AsyncSession = Depends(get_session)):
    ...
    item.status, item.reviewer_id, item.reason, item.decided_at = body.decision, body.reviewer_id, body.reason, datetime.utcnow()
    ...
    session.add(MonitoringEvent(trace_id=item.trace_id, agent_run_id=item.agent_run_id, span_id=item.span_id,
                                ...
                                reason=body.reason or f"Action {body.decision} by {body.reviewer_id}",
                                metadata_json={"reviewer_id": body.reviewer_id, "tool_name": item.tool_name}))
