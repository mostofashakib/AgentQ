#!/usr/bin/env bash
# AgentQ — Demo Mode Runner
# Starts AgentQ with DEMO_MODE=true, which seeds the database with realistic
# sample traces, violations, behavior clusters, and alert rules on first launch.
#
# Usage:
#   ./demo.sh          — start in demo mode (seeds data on first launch)
#   ./demo.sh --reset  — wipe demo data and re-seed before starting
#
# To stop: Ctrl+C
set -euo pipefail
...
RESET_DEMO=false
for arg in "$@"; do
  [[ "$arg" == "--reset" ]] && RESET_DEMO=true
done

log "Starting AgentQ in ${YELLOW}DEMO MODE${RESET}..."
...
if [[ "$RESET_DEMO" == "true" ]]; then
  warn "Resetting demo data..."
  (
    cd "$ROOT"
    set -a; [[ -f .env ]] && source .env; set +a
    DEMO_MODE=true "$ROOT/.venv/bin/python" -c "
import asyncio
...
(
  cd "$ROOT"
  set -a; [[ -f .env ]] && source .env; set +a
  DEMO_MODE=true "$ROOT/.venv/bin/uvicorn" agentq.api.app:app \
...
echo ""
echo -e "  ${BOLD}AgentQ is running in DEMO MODE${RESET}"
...
echo ""
echo -e "  ${YELLOW}Demo data:${RESET} 6 traces · 10 violations · 3 behavior clusters · 2 alert rules"
echo -e "  ${YELLOW}Stored in:${RESET} agentq_demo.db (separate from your real agentq.db — never mixed)"
echo -e "  Re-seed: ${BOLD}./demo.sh --reset${RESET}"
